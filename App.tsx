...
const INITIAL_BOOKS: Book[] = [
  {
    ...
    price: 18.50,
    description: 'Una epopeya de realismo mágico que explorara los lazos familiares a través de las estaciones en un valle olvidado.',
    category: 'Ficción',
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const initData = async () => {
      try {
        ...
        // Carrito sigue siendo local por conveniencia de sesión
  ...
  const saveToLocal = async (newBooks: Book[], newSettings: AppSettings) => {
    try {
      ...
      // Refrescar datos locales para obtener IDs finales
  ...
  const updateOrders = async (newOrders: Order[]) => {
    setOrders(newOrders);
    // Para simplificar, asumimos que solo se actualiza el estado de las órdenes existentes
    // o se añaden nuevas via handleCheckoutWhatsApp
    try {
      // Implementación simplificada: actualizar solo la última o todas
      // En un entorno real, actualizaríamos solo la cambiada
    } catch (error) {
  ...
  const updateReviews = async (newReviews: Review[]) => {
    const oldReviewsCount = reviews.length;
    setReviews(newReviews);
    try {
      if (newReviews.length < oldReviewsCount) {
        // Se eliminó una reseña (identificar cual es complejo sin el ID exacto aquí, 
        // pero se asume que las herramientas del admin manejan el ID de Supabase)
      }
    } catch (error) {
